...
func TestDesanitize(t *testing.T) {
	...
	actual.TelosLoginSettings.Secret = model.NewString("secret")
	...
	target.TelosLoginSettings.Secret = model.NewString(model.FakeSetting)
	...
	assert.Equal(t, *actual.TelosLoginSettings.Secret, *target.TelosLoginSettings.Secret)
